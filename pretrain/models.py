...
class BaseModel(nn.Module):
    def __init__(self,
        ...
        self.build_ema_module('projector', get_mlp, self.backbone.out_dim, 2048, 128)
        self.predictor = get_mlp(128, 2048, 128) if prediction else nn.Identity()
...
class MoCo(BaseModel):
    def __init__(self,
        ...
        super().__init__(backbone=backbone, input_shape=input_shape, momentum=momentum, prediction=prediction)
        self.register_buffer('queue', F.normalize(torch.randn(queue_size, 128)))
        self.register_buffer('queue_ptr', torch.tensor([0]))
        self.temperature = temperature
